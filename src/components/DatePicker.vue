    ...
    <div class="calendar">
      ...
      <div class="days">
        <span v-for="(day, index) in days" :key="index" 
          :class="{ selected: selectedDates.includes(day?.date), disabled: day?.isPastOrToday }"
          @click="!day?.isPastOrToday ? toggleDate(day.date) : null">
          {{ day?.day || '' }}
        </span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    toggleDate(date) {
      ...
      localStorage.setItem('selectedCount', this.selectedDates.length);
      this.error = this.selectedDates.length === 0;
      console.log('Количество выбранных дат:', this.selectedDates.length);
    },
    confirmDates() {
